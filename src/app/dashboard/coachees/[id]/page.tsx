...
export default async function CoacheeDetailPage({ params }: { params: { id: string } }) {
  const supabase = createClient()

  const [{ data: coachee }, { data: sessions }, { data: tasks }] = await Promise.all([
    ...
    <div className="p-8 animate-in">
      ...
      <div className="card mb-6 flex items-start gap-6">
        <div className="w-16 h-16 bg-sage-100 rounded-2xl flex items-center justify-center flex-shrink-0">
          <span className="text-sage-700 text-xl font-semibold font-display">
            {getInitials(coachee.full_name)}
          </span>
        </div>
        <div className="flex-1">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-1">
                <span className="flex items-center gap-1.5 text-sm text-charcoal-400">
                  <Mail className="w-3.5 h-3.5" /> {coachee.email}
                </span>
                {coachee.phone && (
                  <span className="flex items-center gap-1.5 text-sm text-charcoal-400">
                    <Phone className="w-3.5 h-3.5" /> {coachee.phone}
                  </span>
                )}
              </div>
              <p className="text-xs text-charcoal-400 mt-1">
                Cliente desde {formatDate(coachee.created_at)}
              </p>
            </div>
          ...
          {coachee.objectives && (
            <div className="mt-4 p-3 bg-cream-50 rounded-xl">
              <p className="text-xs font-medium text-charcoal-500 mb-1 flex items-center gap-1.5">
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display text-lg font-semibold text-charcoal-800 flex items-center gap-2">
                <CalendarDays className="w-4 h-4 text-sage-500" /> Sessões ({sessions?.length ?? 0})
              </h2>
                ...
                {sessions.map(session => (
                  <Link key={session.id} href={`/dashboard/sessions/${session.id}`}
                    ...
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
                      session.status === 'completed' ? 'bg-sage-400' : 'bg-blue-400'
                    }`} />
                    ...
                    {session.ai_summary && (
                      <span className="text-xs text-violet-500">✨ Resumo</span>
                    )}
                  </Link>
                ))}
              </div>
          ...
          <div className="card">
            ...
            {totalTasks > 0 && (
              <div className="w-full h-1.5 bg-cream-200 rounded-full mb-4">
                <div
                  className="h-1.5 bg-sage-500 rounded-full transition-all"
              ...
              <div className="space-y-2">
                {tasks.slice(0, 6).map(task => (
                  <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-xl">
                    ...
                    <p className={`flex-1 text-sm ${task.status === 'done' ? 'text-charcoal-400 line-through' : 'text-charcoal-700'}`}>
                      {task.title}
                    </p>
        ...
        <div className="col-span-2 space-y-4">
                ...
                {Object.entries(wheelLabels).map(([key, label]) => (
                  <div key={key}>
                    ...
                    <div className="w-full h-1.5 bg-cream-200 rounded-full">
                      <div
                        className="h-1.5 bg-sage-400 rounded-full"
                        style={{ width: `${((wheel[key] ?? 0) / 10) * 100}%` }}
                      />
                    </div>
                  </div>
                ))}
          ...
          <div className="card">
            ...
            <div className="space-y-2 text-sm">
              {[
                { label: 'CPF', value: coachee.cpf },
                { label: 'Telefone', value: coachee.phone },
                { label: 'Bio', value: coachee.bio },
