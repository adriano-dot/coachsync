    ...
    <div className="p-8 max-w-4xl animate-in">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <div className="flex items-center gap-4 text-sm text-charcoal-400">
            <span className="flex items-center gap-1.5">
              <User className="w-3.5 h-3.5" /> {coachee?.full_name}
            </span>
            <span className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-3">
          <Link
            href={`/dashboard/sessions/${session.id}/pdf`}
      ...
      {summary ? (
                ...
                {summary.tasks.map((task, i) => (
                  <div key={i} className={`p-4 rounded-xl border ${
                    ...
                    {task.due_date_suggestion && (
                      <p className="text-xs text-charcoal-400 mt-1">📅 {task.due_date_suggestion}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          {summary.suggested_tools.length > 0 && (
            <div className="card">
              ...
              <div className="grid grid-cols-2 gap-3">
                {summary.suggested_tools.map((tool, i) => (
        ...
        <div className="card text-center py-12">
          ...
          <h3 className="font-display text-lg font-semibold text-charcoal-700 mb-2">
            Nenhuma análise gerada
          </h3>
          <p className="text-charcoal-400 text-sm mb-4">
            Adicione uma transcrição para gerar insights com IA
          </p>
          <Link href={`/dashboard/sessions/${session.id}/edit`} className="btn-primary inline-flex">
