...
export default async function DashboardPage() {
  ...
  const [
    { data: profile },
    { data: coachees, count: coacheesCount },
    { data: sessions },
    { data: tasks },
  ] = await Promise.all([
    supabase.from('profiles').select('*').eq('id', user.id).single(),
    supabase.from('profiles').select('*, count', { count: 'exact' }).eq('coach_id', user.id).eq('role', 'coachee'),
  ...
  const stats = [
    { label: 'Coachees ativos', value: coacheesCount ?? 0, icon: Users, color: 'text-sage-600 bg-sage-50' },
                ...
                >
                  <div className="w-9 h-9 bg-sage-100 rounded-full flex items-center justify-center flex-shrink-0">
                    <span className="text-sage-700 text-xs font-semibold">
                      {(session.coachee as any)?.full_name?.charAt(0) ?? '?'}
                    </span>
                  </div>
